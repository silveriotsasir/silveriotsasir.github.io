var questions = new Array(["¿Sabrías protegerte adecuadamente ante la situación del covid19?", 1, 2],
                          ...
                          ["Lo ideal es haver un lavado con agua y jabón intenso y enérgico durante unos cuantos minutos (recuerda mantener el grifo cerrado mientras frotas) y por ambas caras, o usar un gel hidroalcohólico con una proporción de alcohól superior al 60%. No hace falta hacer ambas cosas: lavarse las manos con agua y jabón y usar gel hidroalcohólico, aunque si se quiere usar las dos cosas no estaría mal del todo", 0, 0]);
var index = 0;

function loadQuestionary(index){
    printQuestion (index);
    printButtons (index);
    let sound = new Audio('sounds/button.wav');
    sound.play();
}
...
function playAmbientSound(){
    console.log("hola");
    let sound = new Audio('sounds/suspense.mp3');
    sound.play();
}